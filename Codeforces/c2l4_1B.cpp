...
*/
LL minn(int L, int R, int treeNode, int qL, int qR)
{
  if (L >= qL && R <= qR)
    return tree[treeNode];
  else if (L > qR || R < qL)
  ...
  else
    {
      int mid = (L + R) / 2;
      LL s = 10000000005;
      s = min(s, minn(L, mid, 2 * treeNode + 1, qL, qR));
      s = min(s, minn(mid + 1, R, 2 * treeNode + 2, qL, qR));
      return s;
    }
}
...
int main()
{
  ...
  for (int i = 0; i < m; ++i)
    {
      cin >> qType >> qA >> qB;
      if (qType == 2) cout << minn(0, n - 1, 0, qA, qB - 1) << endl;
